...
public class ECAlgorithms {
    ...
    public static ECPoint implShamirsTrickWNaf(ECPoint[] eCPointArr, ECPoint[] eCPointArr2, byte[] bArr, ECPoint[] eCPointArr3, ECPoint[] eCPointArr4, byte[] bArr2) {
        ...
        int i2 = 0;
        while (i >= 0) {
            byte b2 = i < bArr.length ? bArr[i] : 0;
            byte b3 = i < bArr2.length ? bArr2[i] : 0;
            if ((b2 | b3) == 0) {
                i2++;
            } else {
                if (b2 != 0) {
                    eCPoint = infinity.add((b2 < 0 ? eCPointArr2 : eCPointArr)[Math.abs(b2) >>> 1]);
                } else {
                    eCPoint = infinity;
                }
                if (b3 != 0) {
                    eCPoint = eCPoint.add((b3 < 0 ? eCPointArr4 : eCPointArr3)[Math.abs(b3) >>> 1]);
                }
                if (i2 > 0) {
        ...
        while (i2 >= 0) {
            ...
            for (int i4 = 0; i4 < r0; i4++) {
                ...
                if (b2 != 0) {
                    ...
                    eCPoint2 = eCPoint2.add(((b2 < 0) == zArr[i4] ? wNafPreCompInfo.getPreComp() : wNafPreCompInfo.getPreCompNeg())[abs >>> 1]);
                }
            }
