    ...
    public static class BaseBehavior<T extends AppBarLayout> extends HeaderBehavior<T> {
        ...
        private int interpolateOffset(@NonNull T t, int i) {
            ...
            while (true) {
                ...
                if (abs < childAt.getTop() || abs > childAt.getBottom()) {
                    ...
                    if (i2 > 0) {
                        float f = (float) i2;
                        int round = Math.round(scrollInterpolator.getInterpolation(((float) (abs - childAt.getTop())) / f) * f);
                        return (childAt.getTop() + round) * Integer.signum(i);
                    }
                }
            }
            return i;
        ...
        public boolean onLayoutChild(@NonNull CoordinatorLayout coordinatorLayout, @NonNull T t, int i) {
            ...
            if (i2 >= 0 && (pendingAction & 8) == 0) {
                ...
                setHeaderTopBottomOffset(coordinatorLayout, t, this.offsetToChildIndexOnLayoutIsMinHeight ? t.getTopInset() + ViewCompat.getMinimumHeight(childAt) + i3 : Math.round(((float) childAt.getHeight()) * this.offsetToChildIndexOnLayoutPerc) + i3);
